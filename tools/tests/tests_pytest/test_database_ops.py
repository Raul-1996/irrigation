...
Uses new API: create_group, create_zone(dict), create_program(dict), create_mqtt_server(dict).
"""
import os
import sys
import json
...
class TestZones:
    ...
    def test_add_zone(self, fresh_db):
        fresh_db.create_group('TestGroup')
        groups = fresh_db.get_groups()
        gid = groups[0]['id']

        result = fresh_db.create_zone({
            'name': 'Test Zone',
            'icon': '🌱',
            'duration': 5,
            'group_id': gid,
    ...
    def test_update_zone(self, fresh_db):
        fresh_db.create_group('G1')
        groups = fresh_db.get_groups()
        gid = groups[0]['id']
        fresh_db.create_zone({
            'name': 'Z1', 'icon': '🌿', 'duration': 1,
            'group_id': gid, 'topic': '/t/k1', 'mqtt_server_id': 1
        })
        zones = fresh_db.get_zones()
        zid = zones[0]['id']
    ...
    def test_delete_zone(self, fresh_db):
        ...
        gid = groups[0]['id']
        fresh_db.create_zone({
            'name': 'ZDel', 'icon': '❌', 'duration': 1,
            'group_id': gid, 'topic': '/t/del', 'mqtt_server_id': 1
        })
        zones = fresh_db.get_zones()
        zid = zones[0]['id']
    ...
    def test_zone_state_update(self, fresh_db):
        ...
        gid = groups[0]['id']
        fresh_db.create_zone({
            'name': 'ZState', 'icon': '💧', 'duration': 1,
            'group_id': gid, 'topic': '/t/state', 'mqtt_server_id': 1
        })
        zones = fresh_db.get_zones()
        zid = zones[0]['id']
...
class TestPrograms:
    ...
    def test_update_program(self, fresh_db):
        fresh_db.create_program({
            'name': 'P1', 'time': '07:00',
            'days': json.dumps([0]), 'zones': json.dumps([1])
        })
        progs = fresh_db.get_programs()
        pid = progs[0]['id']
...
class TestMQTTServers:
    ...
    def test_add_mqtt_server(self, fresh_db):
        fresh_db.create_mqtt_server({
            'name': 'local', 'host': '127.0.0.1', 'port': 1883
        })
        servers = fresh_db.get_mqtt_servers()
    ...
    def test_update_mqtt_server(self, fresh_db):
        fresh_db.create_mqtt_server({
            'name': 'srv', 'host': '10.0.0.1', 'port': 1883
        })
        servers = fresh_db.get_mqtt_servers()
        sid = servers[0]['id']
    ...
    def test_delete_mqtt_server(self, fresh_db):
        fresh_db.create_mqtt_server({
            'name': 'del-srv', 'host': '10.0.0.3', 'port': 1883
        })
        servers = fresh_db.get_mqtt_servers()
        sid = servers[0]['id']
...
class TestLogs:
    ...
    def test_add_log(self, fresh_db):
        # New API: add_log(log_type, details=None)
        fresh_db.add_log('test', 'test log details')
        logs = fresh_db.get_logs()
        assert len(logs) >= 1
...
class TestWaterUsage:
    ...
    def test_add_water_usage(self, fresh_db):
        # New API: add_water_usage(zone_id, liters)
        try:
            fresh_db.add_water_usage(1, 5.0)
        except Exception:
            pass
...
class TestDBInit:
    ...
    def test_concurrent_access(self, tmp_path):
        ...
        db = IrrigationDB(db_path=db_path)
        db.init_database()
        db.create_group('ConcurrentGroup')
