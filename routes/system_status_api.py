        ...
        if group_zones:
            ...
            if group_programs:
                ...
                for program in group_programs:
                    ...
                    program_zones_list = (
                        program["zones"] if isinstance(program["zones"], list) else json.loads(program["zones"])
                    )
                    group_zone_ids = [z["id"] for z in group_zones]
                    ...
                    prog_weekdays = set(
                        int(d)
                        for d in (program["days"] if isinstance(program["days"], list) else json.loads(program["days"]))
                    )
        ...
        for group in groups:
            ...
            try:
                ...
                if usage:
                    ...
                    for zone in zones:
                        zone_usage[str(zone["id"])] = {"name": zone["name"], "liters": 0, "last_used": None}
                    for i in range(7):
                        date = (datetime.now() - timedelta(days=i)).strftime("%Y-%m-%d")
                        daily_usage.append({"date": date, "liters": 0})
                water_data[group_id] = {
                    "group_name": group["name"],
                    "data": {"daily_usage": daily_usage, "total_liters": total_liters, "zone_usage": zone_usage},
                }
            except (sqlite3.Error, OSError) as e:
