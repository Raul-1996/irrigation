...
Rationale: the native production deployment has no authenticating reverse
proxy.  Only fail-safe OFF actions may bypass CSRF; actions that can start
water flow or resume after an emergency require an admin session and CSRF.
...
_ADMIN_ONLY_PATHS_REQUIRING_CSRF = [
    ("POST", "/api/env"),  # environment sensor configuration
...
# Endpoints that MUST remain CSRF-exempt — login, authenticated read-only POSTs,
# and fail-safe OFF actions. Authentication is tested independently.
_CSRF_EXEMPT_POSTS = [
    "/api/login",
    "/api/status",
    "/api/postpone",
...
def test_read_only_and_fail_safe_endpoints_are_exempt(app):
    """All endpoints in _CSRF_EXEMPT_POSTS must be CSRF-exempt."""
    ...
    assert not missing, (
        f"SEC-003 regression: these read-only/fail-safe endpoints are NOT CSRF-exempt: {missing}. "
...
def test_admin_crud_endpoints_are_NOT_exempt(app):
    ...
    forbidden = {
        "api_env_config",  # /api/env POST — changes environment sensor configuration
