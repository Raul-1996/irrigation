...
def test_metrics_contains_at_least_10_required_metrics(client, health_api):
    ...
    assert len(present) >= 10, f"only {len(present)} of {len(REQUIRED_METRICS)} metrics present: {present}"
...
def test_in_flight_gauge_increments_and_decrements(client, health_api):
    ...
    assert gauge._value.get() == baseline, f"in_flight leaked: baseline={baseline}, now={gauge._value.get()}"
...
def test_init_metrics_seeds_log_level_labels(client, health_api):
    ...
    for lvl in ("DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"):
        assert f'wb_logging_records_total{{level="{lvl}"}}' in body, f"level {lvl} not seeded in metrics"
