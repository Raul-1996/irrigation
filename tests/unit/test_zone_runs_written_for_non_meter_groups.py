...
def _has_meter_flag(test_db, group_id):
    ...
    g = next((g for g in (test_db.get_groups() or [])
              if int(g.get('id')) == int(group_id)), None)
...
class TestZoneRunsWrittenForNonMeterGroups:
    def test_start_then_stop_writes_one_ok_run(self, test_db):
        ...
        zone = test_db.create_zone({
            'name': 'NoMeterZone', 'duration': 10, 'group_id': 1,
            'topic': '/test/no-meter',
        })
        ...
        assert len(rows) == 1, (
            f'expected exactly one zone_run for non-meter zone, got {len(rows)}'
        )
        run = rows[0]
