...
class TestSSRRenderingEscaping:
    ...
    def test_script_tag_in_zone_name_escaped(self, rendered_status):
        ...
        if ssr_match:
            ...
            assert "</script>" not in ssr_block.lower(), "Unescaped </script> in SSR data — tojson should escape this"

    def test_img_onerror_in_zone_name_escaped(self, rendered_status):
        """<img src=x onerror=alert(1)> should be escaped in SSR output."""
        html = rendered_status
        assert "<img src=x onerror=alert(1)>" not in html, "Unescaped XSS payload in SSR output"

    def test_xss_in_group_name_escaped(self, rendered_status):
        """Group name with XSS payload should be escaped."""
        html = rendered_status
        assert "<script>alert(document.cookie)</script>" not in html, "Unescaped XSS script tag in group name"
...
class TestInnerHTMLEscaping:
    ...
    def test_status_js_group_name_escaped(self):
        ...
        assert not violations, "status.js has raw group.name in innerHTML:\n" + "\n".join(violations)
    ...
    def test_status_js_zone_name_escaped(self):
        ...
        for i, line in enumerate(lines, 1):
            ...
            is_html_string = ("'" in line or '"' in line or "`" in line) and (
        ...
        assert not violations, "status.js has raw zone.name in innerHTML:\n" + "\n".join(violations)
        ...
        for i, line in enumerate(lines, 1):
            ...
            if "g.name" in line and "escapeHtml" not in line:
                ...
                if is_html_string and ".innerHTML" in "\n".join(lines[max(0, i - 15) : i + 5]):
                    violations.append(f"Line {i}: {stripped[:100]}")
        assert not violations, "status.js has raw g.name in innerHTML:\n" + "\n".join(violations)
    ...
    def test_zones_js_names_escaped(self):
        ...
        name_patterns = [
            ...
            "${group.name}",
            "${s.name}",
            "${c.checked_program_name}",
            "${c.other_program_name}",
        ]
        for i, line in enumerate(lines, 1):
            ...
            for pat in name_patterns:
                if pat in line and "escapeHtml" not in line:
                    ...
                    if ".innerHTML" in ctx or "innerHTML" in ctx:
                        violations.append(f"Line {i}: raw {pat}: {line.strip()[:100]}")
        assert not violations, "mqtt.html has raw server data in innerHTML:\n" + "\n".join(violations)
        ...
        for i, line in enumerate(lines, 1):
            for pat in name_patterns:
                if pat in line and "escapeHtml" not in line:
                    ...
                    if ".innerHTML" in ctx or "innerHTML" in ctx:
                        violations.append(f"Line {i}: raw {pat}: {line.strip()[:100]}")
        assert not violations, "programs.html has raw names in innerHTML:\n" + "\n".join(violations)
