...
class TestRotateAngleClamp:
    ...
    def test_large_angle_rejected(self, admin_client):
        ...
        assert resp.status_code in (400, 404), f"expected 400/404 for huge angle, got {resp.status_code}: {resp.data!r}"
    ...
    def test_valid_angle_accepted_or_photo_missing(self, admin_client):
        ...
        assert resp.status_code in (200, 404), (
            f"valid angle 90 must not be rejected as INVALID_ANGLE (got {resp.status_code}: {resp.data!r})"
        )
