...
def test_import_zones_source_contains_whitelist():
    ...
    assert '_ALLOWED_UPDATE_COLUMNS' in src, (
        'SEC-004 regression: bulk_upsert_zones must keep the explicit column '
        'whitelist in place'
    )
...
def test_import_zones_rejects_unknown_column_in_update(tmp_path):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        ''')
        conn.execute(
            'INSERT INTO zones (id, name, duration, group_id) VALUES (1, "Z1", 10, 1)'
        )
        conn.commit()
...
def test_weather_api_days_source_uses_param_binding():
    ...
    assert 'datetime("now", ?)' in src, (
        'SEC-015 regression: weather_api must use bound parameter for day modifier'
    )
