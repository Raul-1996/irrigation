...
class TestJobStorePersistence:
    ...
    def test_jobs_db_separate_file(self, test_db, tmp_path):
        ...
        try:
            ...
            s.start()
            assert os.path.exists(expected_jobs_db), "jobs.db missing — persistence not writing to dedicated file"
    ...
    def test_misfire_policy_coalesce(self, test_db):
        ...
        try:
            ...
            assert defaults.get("misfire_grace_time", 0) >= 300, (
                "misfire_grace_time must be >= 5 minutes so a late zone_stop still executes after brief restart-gap"
            )
            assert defaults.get("max_instances", 99) == 1, "max_instances=1 prevents the same job running concurrently"
        finally:
            with contextlib.suppress(AttributeError, RuntimeError):
                s.stop()
