    ...
    def test_busy_timeout_set_to_30s(self, test_db_path):
        ...
        try:
            ...
            assert int(row[0]) == 30000, (
                f"PHYS-3: busy_timeout must be 30000ms, got {row[0]}"
            )
        finally:
            conn.close()
    ...
    def test_journal_mode_wal(self, test_db_path):
        ...
        try:
            ...
            assert str(row[0]).lower() == 'wal', (
                f"journal_mode must be WAL, got {row[0]}"
            )
        finally:
            conn.close()
...
class TestFloatRepositoryOps:
    ...
    def test_log_event_persists(self, test_db, test_db_path):
        ...
        ok = repo.log_event(group_id=1, event_type='float_pause',
                            paused_zones=[10, 11])
        assert ok is True

        with sqlite3.connect(test_db_path) as conn:
            row = conn.execute(
                "SELECT group_id, event_type, paused_zones "
                "FROM float_events ORDER BY id DESC LIMIT 1"
            ).fetchone()
    ...
    def test_pause_active_zones_transitions_state(self, test_db, test_db_path):
        ...
        z = test_db.create_zone({
            'name': 'FZ', 'duration': 120, 'group_id': 7,
        })
    ...
    def test_pause_active_zones_ignores_off_zones(self, test_db, test_db_path):
        ...
        z = test_db.create_zone({
            'name': 'OFF', 'duration': 60, 'group_id': 8,
        })
