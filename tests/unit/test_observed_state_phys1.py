...
class TestPhys1Reconciliation:
    ...
    def test_verify_sets_state_fault_after_mqtt_timeout(self, test_db):
        ...
        with patch.object(
            StateVerifier, '_subscribe_and_wait', return_value=False
        ) as mocked_wait, patch(
            'services.events.publish', MagicMock()
        ), patch(
            'services.mqtt_pub.publish_mqtt_value', MagicMock()
        ):
            result = sv.verify(zone_id, 'on', timeout=0.01,
                               retries=OBSERVED_STATE_MAX_RETRIES)
        ...
        assert mocked_wait.call_count == OBSERVED_STATE_MAX_RETRIES, (
            f"expected {OBSERVED_STATE_MAX_RETRIES} MQTT wait attempts, "
            f"got {mocked_wait.call_count}"
        )

        # Contract: zone is pinned to state='fault'.
        after = test_db.get_zone(zone_id)
        assert after.get('state') == 'fault', (
            f"PHYS-1 violated: expected state='fault', got {after.get('state')!r}"
        )
        assert int(after.get('fault_count') or 0) == initial_faults + 1, (
            "fault_count must be incremented"
        )
    ...
    def test_verify_success_does_not_mark_fault(self, test_db):
        ...
        after = test_db.get_zone(zone_id)
        assert after.get('state') != 'fault', \
            "successful verify must NOT mark zone as fault"
    ...
    def test_verify_retries_publish_between_attempts(self, test_db):
        ...
        with patch.object(
            ...
            'services.mqtt_pub.publish_mqtt_value'
        ) as mocked_publish:
            sv.verify(zone_id, 'on', timeout=0.01,
                      retries=OBSERVED_STATE_MAX_RETRIES)
        ...
        assert mocked_publish.call_count == expected_publishes, (
            f"expected {expected_publishes} retry-publishes, "
            f"got {mocked_publish.call_count}"
        )
