...
class TestSkipEndpointAPI:
    def test_skip_no_active_session_returns_400(self, admin_client, app):
        ...
        with patch("routes.groups_api.get_scheduler", return_value=sch):
            resp = admin_client.post("/api/groups/1/skip-current", content_type="application/json", data="{}")
    ...
    def test_skip_group_not_found_returns_404(self, admin_client, app):
        ...
        resp = admin_client.post("/api/groups/9999/skip-current", content_type="application/json", data="{}")
...
class TestSkipSequencerLoop:
    ...
    def test_skip_advances_to_next_zone(self, app):
        ...
        try:
            ...
            assert poll_until(lambda: test_db.get_zone(z1["id"])["state"] == "on", timeout=3.0), "zone1 never started"

            # Fire skip
            assert sch.request_skip_current_zone(1) == "ok"

            # Outcome: zone1 stops AND zone2 starts within ~2s of skip
            assert poll_until(lambda: test_db.get_zone(z1["id"])["state"] == "off", timeout=3.0), (
                "zone1 did not stop after skip"
            )
            assert poll_until(lambda: test_db.get_zone(z2["id"])["state"] == "on", timeout=3.0), (
                "zone2 did not start after skip"
            )
            ...
            try:
                ...
                logs = []
            assert any(json.loads(l.get("details") or "{}").get("zone_id") == z1["id"] for l in logs), (
    ...
    def test_skip_double_click_advances_only_once(self, admin_client, app):
        ...
        try:
            ...
            with patch("routes.groups_api.get_scheduler", return_value=sch):
                t0 = time.monotonic()
                resp1 = admin_client.post("/api/groups/1/skip-current", content_type="application/json", data="{}")
                resp2 = admin_client.post("/api/groups/1/skip-current", content_type="application/json", data="{}")
            ...
            # second time).
            assert poll_until(lambda: test_db.get_zone(z2["id"])["state"] == "on", timeout=3.0), "zone2 did not start"
            time.sleep(0.5)
            assert test_db.get_zone(z3["id"])["state"] == "off", "double-skip incorrectly advanced past zone2 to zone3"
    ...
    def test_skip_current_program_watering_works(self, admin_client, app):
        ...
        try:
            ...
            assert poll_until(lambda: test_db.get_zone(z1["id"])["state"] == "on", timeout=5.0), (
                "zone1 never started in program run"
            )

            # The C1 fix: is_group_session_active(1) must now be True
            # because _run_program_threaded pre-registered the cancel event.
            assert sch.is_group_session_active(1), "C1 regression: program run did not register group_cancel_events"
            ...
            assert poll_until(lambda: test_db.get_zone(z1["id"])["state"] == "off", timeout=5.0), (
                "zone1 did not stop after skip in program run"
            )
            assert poll_until(lambda: test_db.get_zone(z2["id"])["state"] == "on", timeout=5.0), (
                "zone2 did not start after skip in program run"
            )
    ...
    def test_skip_event_cleared_in_finally(self, app):
        ...
        try:
            ...
            t.start()
            assert poll_until(lambda: test_db.get_zone(z1["id"])["state"] == "on", timeout=3.0)
            ...
            assert 1 not in sch.group_skip_current_events, "group_skip_current_events not cleaned in finally"
            assert 1 not in sch.group_cancel_events, "group_cancel_events not cleaned in finally"
        finally:
            for p in patches:
                with contextlib.suppress(Exception):
                    p.stop()
