...
class TestBootSyncAbortsStaleRuns:
    def test_open_run_marked_aborted_on_boot(self, test_db):
        ...
        zone = test_db.create_zone({
            'name': 'Stale', 'duration': 10, 'group_id': 1,
        })
        ...
        assert test_db.finish_zone_run(
            int(prior), '2026-04-01 09:15:00', 1.0, None, None, None,
            status='ok',
        )
        # Seed the stale open run (simulating a crash mid-watering).
        stale_id = test_db.create_zone_run(
            int(zone['id']), 1, '2026-04-02 10:00:00', 0.0, None, 1, None,
        )
        ...
        with _patch.object(_zc, 'stop_all_in_group',
                           return_value=None), \
        ...
        assert row[0] == 'aborted', (
            f"stale open run should be aborted, got status={row[0]!r}"
        )
    ...
    def test_already_finished_run_left_alone(self, test_db):
        ...
        """
        zone = test_db.create_zone({
            'name': 'Closed', 'duration': 10, 'group_id': 1,
        })
        ok_id = test_db.create_zone_run(
            int(zone['id']), 1, '2026-04-01 09:00:00', 0.0, None, 1, None,
        )
        assert test_db.finish_zone_run(
            int(ok_id), '2026-04-01 09:15:00', 1.0, None, None, None,
            status='ok',
        )
        ...
        import services.zone_control as _zc
        import services.mqtt_pub as _mp
        with _patch.object(_zc, 'stop_all_in_group', return_value=None), \
             _patch.object(_mp, 'publish_mqtt_value', return_value=True):
            app_init._boot_sync(MagicMock(), test_db)
