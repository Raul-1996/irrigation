...
def test_group_sequence_opens_and_closes_zone_run(runner_db):
    ...
    with sqlite3.connect(runner_db.db_path) as conn:
        rows = conn.execute("SELECT zone_id, group_id, status, end_utc FROM zone_runs WHERE zone_id = 1").fetchall()
    assert len(rows) == 1, f"expected exactly one zone_run row written by _run_group_sequence, got {len(rows)}"
