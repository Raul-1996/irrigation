...
def test_manual_program_run_ignores_weather_skip(runner_db):
    ...
    with (
        patch.object(sched, "_check_weather_skip", return_value={"skip": True, "reason": "heavy_rain"}) as mock_skip,
    ...
    assert mock_start.call_count >= 1, "manual=True must bypass weather skip and reach exclusive_start_zone"
...
def test_scheduled_program_run_still_respects_weather_skip(runner_db):
    ...
    with (
        patch.object(sched, "_check_weather_skip", return_value={"skip": True, "reason": "heavy_rain"}),
        patch("services.zone_control.exclusive_start_zone", return_value=True) as mock_start,
        patch("services.zone_control.publish_mqtt_value", return_value=True),
        patch("services.zone_control.db", runner_db),
        patch("services.mqtt_pub.publish_mqtt_value", return_value=True),
    ...
    assert mock_start.call_count == 0, "scheduled run (manual=False) must respect weather skip and NOT start zones"
...
def test_manual_group_sequence_ignores_weather_skip(runner_db):
    ...
    with sqlite3.connect(runner_db.db_path) as conn:
        rows = conn.execute("SELECT zone_id FROM zone_runs WHERE zone_id = 1").fetchall()
    assert len(rows) >= 1, "manual=True group_sequence must bypass weather skip and open a zone_run"
