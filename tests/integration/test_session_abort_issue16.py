...
class TestSessionAbortIssue16:
    def test_full_watering_cycle_zone_stop_aborts_sequence(self, admin_client, app):
        ...
        assert sched.is_group_session_active(group["id"]), (
            "precondition: start_group_sequence must have registered a cancel-event for the group"
        )
        ...
        resp = admin_client.post(f"/api/zones/{z1['id']}/mqtt/stop", content_type="application/json")
    ...
    def test_solo_zone_stop_unchanged_behaviour(self, admin_client, app):
        ...
        assert all(r.get("target") != f"group:{group['id']}" for r in rows), (
