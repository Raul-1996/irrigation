...
def test_zero_coef_skips_zone(runner_db):
    ...
    with (
        patch("services.weather_adjustment.get_weather_adjustment") as mock_wa,
...
def _mock_weather_data(temperature=22.0, humidity=55.0, precipitation_24h=0.5, wind_speed=3.0):
    m = MagicMock()
...
def test_program_writes_weather_log(runner_db):
    ...
    with (
        patch("services.weather.adjustment.WeatherAdjustment._get_weather", return_value=weather),
    ...
    zone_id, orig, _adj, coef, wdata = row
    assert zone_id == 1
    assert orig == 15
    assert coef == 80
...
def test_program_writes_weather_decision(runner_db):
    ...
    conn = sqlite3.connect(runner_db.db_path)
    cur = conn.execute(
        "SELECT decision, coefficient, temperature, mode FROM weather_decisions ORDER BY id DESC LIMIT 1"
    )
    row = cur.fetchone()
    conn.close()
