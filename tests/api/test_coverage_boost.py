...
class TestGroupOperations:
    ...
    def test_group_create_and_operations(self, admin_client, app):
        # Create group
        resp = admin_client.post(
            "/api/groups", data=json.dumps({"name": "TestOps Group"}), content_type="application/json"
        )
        assert resp.status_code in (200, 201)
...
class TestSettingsOperations:
    ...
    def test_early_off_set(self, admin_client):
        resp = admin_client.post(
            "/api/settings/early-off", data=json.dumps({"seconds": 3}), content_type="application/json"
        )
        assert resp.status_code == 200

    def test_system_name_set(self, admin_client):
        resp = admin_client.post(
            "/api/settings/system-name", data=json.dumps({"name": "Irrigator Pro"}), content_type="application/json"
        )
        assert resp.status_code == 200
    ...
    def test_rain_config_set(self, admin_client):
        resp = admin_client.post("/api/rain", data=json.dumps({"enabled": False}), content_type="application/json")
        assert resp.status_code == 200

    def test_env_config_set(self, admin_client):
        resp = admin_client.post("/api/env", data=json.dumps({}), content_type="application/json")
        assert resp.status_code == 200

    def test_logging_debug_set(self, admin_client):
        resp = admin_client.post(
            "/api/logging/debug", data=json.dumps({"enabled": False}), content_type="application/json"
        )
        assert resp.status_code == 200
