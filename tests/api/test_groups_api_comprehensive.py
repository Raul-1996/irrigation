...
class TestGroupsAPI:
    ...
    def test_create_group(self, admin_client):
        resp = admin_client.post("/api/groups", data=json.dumps({"name": "New Group"}), content_type="application/json")
        assert resp.status_code in (200, 201)

    def test_create_group_empty_name(self, admin_client):
        resp = admin_client.post("/api/groups", data=json.dumps({"name": ""}), content_type="application/json")
    ...
    def test_update_group(self, admin_client, app):
        ...
        resp = admin_client.put(
            f"/api/groups/{g['id']}", data=json.dumps({"name": "Updated"}), content_type="application/json"
        )
...
class TestGroupSequenceAPI:
    def test_start_group_sequence(self, admin_client, app):
        ...
        resp = admin_client.post(f"/api/groups/{g['id']}/start-from-first", content_type="application/json")
...
class TestGroupSequencePercent:
    def test_start_group_percent_per_zone(self, admin_client, app):
        ...
        resp = admin_client.post(
            f"/api/groups/{g['id']}/start-from-first",
    ...
    def test_start_group_percent_warnings_propagated(self, admin_client, app):
        ...
        assert isinstance(warns, list), f"group endpoint must return warnings[] (Issue #12 C1), got: {body!r}"
        assert "norm_not_set" in warns, f"zone with duration<=0 must produce 'norm_not_set' warning, got: {warns!r}"
