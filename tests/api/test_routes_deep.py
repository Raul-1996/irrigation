...
class TestMqttStartStop:
    def test_mqtt_start(self, admin_client, app):
        ...
        resp = admin_client.post(f"/api/zones/{z['id']}/mqtt/start", content_type="application/json")
        assert resp.status_code in (200, 400, 500)

    def test_mqtt_start_not_found(self, admin_client):
        resp = admin_client.post("/api/zones/99999/mqtt/start", content_type="application/json")
        assert resp.status_code == 404

    def test_mqtt_stop(self, admin_client, app):
        z = app.db.create_zone({"name": "MS", "duration": 10, "group_id": 1, "topic": "/t/z"})
        app.db.update_zone(z["id"], {"state": "on", "watering_start_time": "2026-01-01 10:00:00"})
        resp = admin_client.post(f"/api/zones/{z['id']}/mqtt/stop", content_type="application/json")
        assert resp.status_code in (200, 400, 500)

    def test_mqtt_stop_not_found(self, admin_client):
        resp = admin_client.post("/api/zones/99999/mqtt/stop", content_type="application/json")
        assert resp.status_code == 404
...
class TestPhotoEndpoints:
    ...
    def test_upload_photo(self, admin_client, app):
        ...
        resp = admin_client.post(
            f"/api/zones/{z['id']}/photo", data={"photo": (data, "test.jpg")}, content_type="multipart/form-data"
        )
        assert resp.status_code in (200, 400, 500)
    ...
    def test_upload_bad_extension(self, admin_client, app):
        ...
        resp = admin_client.post(
            f"/api/zones/{z['id']}/photo", data={"photo": (data, "test.exe")}, content_type="multipart/form-data"
        )
    ...
    def test_rotate_photo_no_photo(self, admin_client, app):
        ...
        resp = admin_client.post(
            f"/api/zones/{z['id']}/photo/rotate", data=json.dumps({"angle": 90}), content_type="application/json"
        )
        assert resp.status_code in (200, 404)
