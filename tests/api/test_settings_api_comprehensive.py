...
class TestSettingsPage:
    ...
    def test_settings_page_post(self, admin_client):
        resp = admin_client.post(
            "/settings", data={"zone_cap_minutes": "120"}, content_type="application/x-www-form-urlencoded"
        )
...
class TestPasswordChange:
    def test_change_password(self, admin_client):
        resp = admin_client.post(
            "/settings/password", data=json.dumps({"new_password": "NewSecure123!"}), content_type="application/json"
        )
