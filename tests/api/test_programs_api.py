...
class TestProgramsAPI:
    ...
    def test_create_program_missing_fields(self, admin_client):
        resp = admin_client.post(
            "/api/programs", data=json.dumps({"name": "Incomplete"}), content_type="application/json"
        )
...
class TestCheckConflicts:
    ...
    def test_check_conflicts_missing_data(self, admin_client):
        resp = admin_client.post(
            "/api/programs/check-conflicts", data=json.dumps({"time": "06:00"}), content_type="application/json"
        )
        assert resp.status_code == 400
