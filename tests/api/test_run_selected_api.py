...
class TestRunSelectedValidation:
    ...
    def test_zone_not_in_group(self, admin_client, app):
        ...
        gid_a, _zids_a = _create_group_with_zones(app, n=1, durations=[10])
        _gid_b, zids_b = _create_group_with_zones(app, n=1, durations=[10])
...
class TestManualVsScheduledGuard:
    def test_scheduled_blocked_by_active_manual(self, admin_client, app):
        ...
        try:
            ...
            n_skipped_before = sum(
                1 for r in logs_before if (r.get("type") or r.get("action")) == "prog_skipped_manual_running"
            )
            ...
            n_skipped_after = sum(
                1 for r in logs_after if (r.get("type") or r.get("action")) == "prog_skipped_manual_running"
            )
            assert n_skipped_after == n_skipped_before + 1, (
                f"expected one prog_skipped_manual_running, got {n_skipped_after - n_skipped_before}"
            )
