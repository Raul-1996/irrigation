...
class TestLoginAPI:
    ...
    def test_login_empty_password(self, client):
        resp = client.post("/api/login", data=json.dumps({"password": ""}), content_type="application/json")
        assert resp.status_code == 401

    def test_login_rate_limit(self, client):
        """After many failed attempts, should get 429."""
        for _ in range(15):
            client.post("/api/login", data=json.dumps({"password": "wrong"}), content_type="application/json")
        resp = client.post("/api/login", data=json.dumps({"password": "wrong"}), content_type="application/json")
