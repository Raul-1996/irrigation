...
class TestZoneCRUD:
    ...
    def test_create_zone(self, admin_client):
        resp = admin_client.post(
            "/api/zones",
            data=json.dumps({"name": "Func Test Zone", "duration": 15, "group_id": 1, "icon": "🌿"}),
            content_type="application/json",
        )
...
class TestGroupOperations:
    ...
    def test_cancel_postpone(self, admin_client):
        ...
        # Cancel
        resp = admin_client.post(
            "/api/postpone", data=json.dumps({"group_id": gid, "action": "cancel"}), content_type="application/json"
        )
        assert resp.status_code == 200
...
class TestWeatherAndNextWatering:
    ...
    def test_next_watering_bulk_with_zones(self, admin_client):
        ...
        for i in range(3):
            admin_client.post(
                "/api/zones",
                data=json.dumps({"name": f"NW Test {i}", "duration": 10, "group_id": 1}),
                content_type="application/json",
            )
        resp = admin_client.get("/api/zones")
        ...
        resp = admin_client.post(
            "/api/zones/next-watering-bulk", data=json.dumps({"zone_ids": ids}), content_type="application/json"
        )
        assert resp.status_code == 200
        data = json.loads(resp.data)
        assert "items" in data
    ...
    def test_next_watering_single(self, admin_client):
        admin_client.post(
            "/api/zones",
            data=json.dumps({"name": "NW Single", "duration": 10, "group_id": 1}),
            content_type="application/json",
        )
        resp = admin_client.get("/api/zones")
        zones = json.loads(resp.data)
...
class TestZoneFieldsComplete:
    ...
    def test_zone_has_required_fields(self, admin_client):
        admin_client.post(
            "/api/zones",
            data=json.dumps({"name": "Fields Test", "duration": 15, "group_id": 1, "icon": "🌹"}),
            content_type="application/json",
        )
        resp = admin_client.get("/api/zones")
        zones = json.loads(resp.data)
        assert len(zones) > 0
        z = zones[0]
        for field in ["id", "name", "duration", "group_id", "state", "icon", "photo_path", "last_watering_time"]:
