...
def _local_iso_at(local_hour: int, local_minute: int, days_back: int = 0) -> str:
    ...
    return target_local.astimezone(timezone.utc).isoformat().replace('+00:00', 'Z')
...
class TestZoneRunsSourceMigration:
    ...
    def test_index_present(self, test_db_path):
        _make_db(test_db_path)
        conn = sqlite3.connect(test_db_path)
        idx_names = {row[1] for row in conn.execute(
    ...
    def test_backfill_marks_matching_run_as_program(self, test_db_path, monkeypatch):
        ...
        db = _make_db(test_db_path)
        zone = db.create_zone({
            'name': 'Z1', 'duration': 15, 'group_id': 1,
            'topic': '/devices/t/K1',
        })
        ...
        run_dt = (now_local - timedelta(days=days_back)).replace(
            hour=7, minute=0, second=30, microsecond=0,
        )
        ...
        start_utc = run_dt.astimezone(timezone.utc).isoformat().replace('+00:00', 'Z')
        ...
        conn.execute(
            "DELETE FROM migrations WHERE name = 'zone_runs_backfill_source'"
        )
        conn.commit()
        conn.close()
        ...
        conn = sqlite3.connect(test_db_path)
        row = conn.execute(
            "SELECT source FROM zone_runs WHERE start_utc = ?", (start_utc,)
        ).fetchone()
        conn.close()
    ...
    def test_backfill_handles_null_start_utc(self, test_db_path):
        ...
        conn = sqlite3.connect(test_db_path)
        row = conn.execute(
            "SELECT source FROM zone_runs WHERE start_utc IS NULL"
        ).fetchone()
        conn.close()
        assert row is not None
        assert row[0] == 'manual'
    ...
    def test_idempotent_rerun(self, test_db_path):
        ...
        conn = sqlite3.connect(test_db_path)
        row = conn.execute(
            "SELECT source FROM zone_runs WHERE zone_id = ?", (zone['id'],)
        ).fetchone()
        conn.close()
